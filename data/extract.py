...
def get_ascii_sequences(filename):
    ...
    lines = [stroke_utils.encode_ascii(
        line)[:stroke_utils.MAX_CHAR_LEN] for line in lines]
    return lines
...
def collect_data(DEBUG=False):
    ...
    for i, fname in enumerate(fnames):
        ...
        original_xml = os.path.join(
            original_dir, 'strokes' + last_letter + '.xml')
        ...
        for ascii_seq, line_stroke_fname in zip(ascii_sequences, line_stroke_fnames):
            ...
            stroke_fnames.append(os.path.join(
                line_stroke_dir, line_stroke_fname))
